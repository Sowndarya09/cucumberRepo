...
public class ConsumerDrivenHealthPlan extends Driver {
	...
	@FindBy(xpath = "//a[@data-test-id='201710161647390047298787']")
	...
	/**
	 * Constructor for the ConsumerDrivenHealthPlan class defining the Iframe and
	 * the Page Factory
	 */
	public ConsumerDrivenHealthPlan() throws IOException {
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateCDHPViewPage Type:Textbox
	 * Description:This method verifies if headers and links - Anthem CDH,
	 * Tier1/Tier2 Document, CAD links are properly displayed when page is loaded
	 */
	public boolean validateCDHPViewPage() {
		...
		try {
			if (utils.validateHeader(this.sHeaderCDHP, "View Consumer Driven Health Plan Accounts")) {
				if (this.anthemCDHLink.isDisplayed() && this.tier1_tier2DocLink.isDisplayed()
						&& this.cadLink.isDisplayed()) {
					blogger.loginfo(
							"The following links are displayed on CDHPView page - Anthem CDH, Tier1/Tier2 Document, CAD");
					return true;
				}
				err.logcommonMethodError("ConsumerDrivenHealthPlan",
						"Links are not displayed-Anthem CDH, Tier1/Tier2 Document, CAD");
				return false;
			}
			err.logcommonMethodError("ConsumerDrivenHealthPlan", "validateCDHPViewPage - Header doesnt match on page");
			return false;
		} catch (Exception e) {
			err.logError("validateCDHPViewPage - CDHP page isnt loaded ");
			blogger.loginfo(
					"Exception occured while trying to load - View Consumer Driven Health Plan Accounts screen." + e);
			return false;
		}
	}

	/*
	 * @SCU #CommonMethodwithoutArgument:verifyDisplayOfTier1OrTier2DocumentLink
	 * Type:TextBox Description: This method verifies if Tier1/Tier2 Document link
	 * is displayed on the "View Consumer Driven Health Plan Accounts" page and user
	 * clicks on it - to navigate Knowledge Library which would help CSR to answer
	 * CDHP related inquiries coming from member on call.
	 */
	public boolean verifyDisplayOfTier1OrTier2DocumentLink() throws InterruptedException, AWTException {
		...
		if (utils.validateHeader(this.sHeaderCDHP, "View Consumer Driven Health Plan Accounts")) {
			if (this.tier1_tier2DocLink.isDisplayed()) {
				...
				System.out.println(
						"Tier1 / Tier2 Document link is added to 'ConsumerDrivenHealthPlan' task and user clicks the link");
				Robot ro = new Robot();
				Thread.sleep(10000);
				...
				if (title.contains("Anthem Tier 1 and Tier 2 Calls Chart")) {
					...
					err.logcommonMethodError("ConsumerDrivenHealthPlan",
							"Error in switching to childwindow-Tier1/Tier2");
					return false;
				}
			}
			err.logcommonMethodError("ConsumerDrivenHealthPlan",
					"Tier1 / Tier2 Document link is not displayed in - View Consumer Driven Health Plan Accounts");
			return false;
		}
		err.logcommonMethodError("ConsumerDrivenHealthPlan",
				"View Consumer Driven Health Plan Accounts page is not loaded");
		return false;
	}

	/*
	 * @SCU #CommonMethodwithoutArgument:verifyDisplayOfTier1OrTier2DocumentLinkIsDisplayed
	 * Type:TextBox Description: This method verifies if Tier1/Tier2 Document link
	 * is displayed on the "View Consumer Driven Health Plan Accounts" page and user
	 * clicks on it - to navigate Knowledge Library which would help CSR to answer
	 * CDHP related inquiries coming from member on call.
	 */
	public boolean verifyDisplayOfTier1OrTier2DocumentLinkIsDisplayed() throws InterruptedException, AWTException {
		String parent = Driver.pgDriver.getWindowHandle();
		if (utils.validateHeader(this.sHeaderCDHP, "View Consumer Driven Health Plan Accounts"))
			if (this.tier1_tier2DocLink.isDisplayed())
				return utils.clickAnelemnt(this.tier1_tier2DocLink, "ConsumerDrivenHealthPlan", "Tier1 / Tier2 Document");
		return false;
	}


	/*
	 * @SCU #CommonMethodwithoutArgument:verifyDisplayOfCADLink Type:TextBox
	 * Description: This method verifies if 'Consumer Access Database (CAD)' link is
	 * displayed on the "View Consumer Driven Health Plan Accounts" page and user
	 * clicks on it - to navigate to the CAD (Anthem.com Emulator) tool so that CSR
	 * can assist a member with online navigation steps to access their CDHP
	 * details.
	 */
	...
	/*
	 * @SCU #CommonMethodwithArgument:validateCDHPAccountsColHeader
	 * #Arguments:Column headers should be passed as comma separated parameters as
	 * in order of UI
	 * -"Account Description,Status,Effective Date,Plan Start Date,Plan End Date".
	 * Type:Text Description: This method verifies that if Consumer Driven Health
	 * Plan Accounts - table grid is displayed and its column headers are validated.
	 */
	public boolean validateCDHPAccountsColHeader(String[] args) {
		...
		if (this.lblCDHPAccounts.isDisplayed() && this.CDHPAccountsLevelTable.isDisplayed()) {
			if (utils.validateTableRowHeader(this.CDHPAccountsLevelTable, colHdrs)) {
				return !utils.isProxyWebelement(noResultsGrid);
			}
		}
		return false;
	}
	...
	/*
	 * @SCU #CommonMethodwithArgument:selectCDHPAccountLevelInfo #Description:This
	 * method selects a particular 'Consumer Driven Health Plan Account Info',
	 * depending on the parameters passed for selection.
	 * #Arguments:CDHPAccountLevel-KeyValuePair Type:Table Keys:#Account
	 * Description#Status#Effective Date#Plan Start Date#Plan End Date
	 */
	public boolean selectCDHPAccountLevelInfo(String[] args) throws InterruptedException {
		...
		for (String iterator : args) {
			...
			if (!returnvar) {
				err.logcommonMethodError("selectCDHPAccountLevelInfo", "Check your " + keyvaluepair
		...
		try {
			if (utils.clickontablerowbasedonvalues(this.CDHPAccountsLevelTable, args)) {
				if (this.lblSummaryTab.isDisplayed()) {
					System.out.println(
							"Summary Tab is displayed by default on page load - When user clicks on CDHP Account level info");
				} else
					System.out
					.println("Summary Tab is not displayed by default on page load - CDHP Account level info");
			}
		}
	...
	/*
	 * @SCU #CommonMethodwithArgument:validateSummaryTabInCDHPAccountPlan
	 * #Description:This method validated the fields in Summary Tab for a particular
	 * 'Consumer Driven Health Plan Account Info'.
	 * #Arguments:CDHPAccountLevelSummaryTab-KeyValuePair Type:Table
	 * Keys:#acct_description#acct_type#status#eff#start#end#ld_of_spending#
	 * ld_for_submitclaims#fund_rollover_rule#fund_rollover_percentage#
	 * fund_rollover_cap#fund_rollover_date#annual_election#contribution_ytd#
	 * incentives_deposits#rollover_deposit#available_balance#holds#
	 * disbursable_balance#balance_due#member_pay#contribution_ytd
	 */
	public boolean validateSummaryTabInCDHPAccountPlan(String[] args) {
		...
		utils.waitforpageload();
		boolean returnvar;
		String acct_description = "", acct_type = "", status = "", eff = "", start = "", end = "", ld_of_spending = "",
				ld_for_submitclaims = "";
		...
		if (utils.validateLabel(this.lblSummaryTab, "Summary")) {
			...
			for (String iterator : args) {
				if (!returnvar) {
					err.logcommonMethodError("validateSummaryTabInCDHPAccountPlan", "Check your " + keyvaluepair
							+ " Either Your input is wrong or the value found on application is incorrect");
					return false;
				...
				if (key.equalsIgnoreCase("acct_description")) {
					...
					try {
						returnvar = this.txt_acct_description.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Account Description' CDHP - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_acct_type.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Account Type' CDHP - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						...
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Status'in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						...
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Effective Date' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						...
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Plan Start Date' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						...
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Plan End Date' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_ld_of_spending.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Last Date of Spending' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_ld_for_submitclaims.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Last Date To Submit Claims' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_fund_rollover_rule.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Fund RollOver Rule' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_fund_rollover_percentage.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Fund RollOver Percentage' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_fund_rollover_cap.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Fund RollOver CAP' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_fund_rollover_date.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Fund RollOver Date' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_annual_election.getText().toLowerCase().trim().replace(",", "")
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Annual Election' in - Summary Tab");
						return false;
					}
					continue;
					...
					annual_election_contribution_ytd = value.trim();
					try {
						returnvar = this.txt_contribution_ytd.getText().toLowerCase().trim().replace(",", "")
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Contribution YTD' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_incentives_deposits.getText().toLowerCase().trim().replace(",", "")
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Incentives Deposits' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_rollover_deposit.getText().toLowerCase().trim().replace(",", "")
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'RollOver Deposit' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_available_balance.getText().toLowerCase().trim().replace(",", "")
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Available Balance' in - Summary Tab");
						return false;
					}
					continue;
					...
					holds = value.trim();
					try {
						returnvar = this.txt_holds.getText().toLowerCase().trim().replace(",", "")
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Holds' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_disbursable_balance.getText().toLowerCase().trim().replace(",", "")
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Disbursable Balance' in - Summary Tab");
						return false;
					}
					continue;
					...
					try {
						returnvar = this.txt_balance_due.getText().toLowerCase().trim().replace(",", "")
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Balance Due' in - Summary Tab");
						return false;
					}
					continue;
				} else
	 ...
	 * validateSectionsDisplayedInSummaryTabOfCDHPAccount #Description:This method
	 * validated the labels and sections displayed in Summary Tab for a particular
	 * 'Consumer Driven Health Plan Account Info'. Type:TextBox
	 */
	public boolean validateSectionsDisplayedInSummaryTabOfCDHPAccount() {
		...
		try {
			if (this.lbl_acct_description.isDisplayed()) {
				System.out.println("Account Description is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_acct_type.isDisplayed()) {
		...
		try {
			if (this.lbl_status.isDisplayed()) {
		...
		try {
			if (this.lbl_eff.isDisplayed()) {
				System.out.println("Effective Date is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_start.isDisplayed()) {
				System.out.println("Plan Start Date is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_end.isDisplayed()) {
				System.out.println("Plan End Date is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_ld_of_spending.isDisplayed()) {
				System.out.println("Last date of Spending is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Summary tab - Last date of Spending label is not displayed");
			System.out.println("Last date of Spending label is not displayed" + e);
			fieldsNotPresent.add("Last date of Spending");
		}

		try {
			if (this.lbl_ld_for_submitclaims.isDisplayed()) {
				System.out.println("Last Date to Submit Claims is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Summary tab - Last date to Submit Claims label is not displayed");
			System.out.println("Last date to Submit Claims label is not displayed" + e);
			fieldsNotPresent.add("Last date to Submit Claims");
		}
		((JavascriptExecutor) Driver.getPgDriver()).executeScript("arguments[0].scrollIntoView(true);",
				this.lbl_RolloverInformation_section);
		try {
			if (this.lbl_fund_rollover_rule.isDisplayed()) {
				System.out.println("Fund RollOver Rule is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_fund_rollover_percentage.isDisplayed()) {
				System.out.println("Fund RollOver Percentage is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Summary tab - Fund RollOver Percentage- label is not displayed");
			System.out.println("Fund RollOver Percentage- label is not displayed" + e);
			fieldsNotPresent.add("Fund RollOver Percentage");
		}

		try {
			if (this.lbl_fund_rollover_cap.isDisplayed()) {
				System.out.println("Fund RollOver Cap is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_fund_rollover_date.isDisplayed()) {
				System.out.println("Fund RollOver date is displayed");
			}
		} catch (Exception e) {
		...
		((JavascriptExecutor) Driver.getPgDriver()).executeScript("arguments[0].scrollIntoView(true);",
				this.lbl_ContributionInformation_section);
		try {
			if (this.lbl_annual_election.isDisplayed()) {
				System.out.println("Annual Election is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_contribution_ytd.isDisplayed()) {
				System.out.println("Contribution YTD is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_incentives_deposits.isDisplayed()) {
				System.out.println("Incentives Deposits is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_rollover_deposit.isDisplayed()) {
				System.out.println("Rollover Deposit is displayed");
			}
		} catch (Exception e) {
		...
		((JavascriptExecutor) Driver.getPgDriver()).executeScript("arguments[0].scrollIntoView(true);",
				this.lbl_BalanceInformation_section);
		try {
			if (this.lbl_available_balance.isDisplayed()) {
				System.out.println("Available Balance is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_holds.isDisplayed()) {
				System.out.println("Holds is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_disbursable_balance.isDisplayed()) {
				System.out.println("Disbursable Balance is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_balance_due.isDisplayed()) {
				System.out.println("Balance Due is displayed");
			}
		} catch (Exception e) {
	...
	/*
	 * @SCU #CommonMethodwithArgument:verifyTransactionTableColHeader #Description:
	 * This method verifies that column headers in Transactions sub-tables are
	 * displayed, when user clicks on Transactions tab in CDHP Account Plan.
	 * #Arguments:Transaction table headers
	 * -"Service Start Date,Transaction Date,Account,Claimant,Claim # - Claim Line,Type,Description,Status,Transaction Amount"
	 * . Type:TextBox
	 */
	public boolean verifyTransactionTableColHeader(String[] args) {
		...
		StringBuilder input = new StringBuilder();
		String colHdrs[] = input.append(space).append(",").append(args[0]).append(",").append(args[1]).append(",")
				.append(args[2]).append(",").append(args[3]).append(",").append(args[4]).append(",").append(args[5])
				.append(",").append(args[6]).append(",").append(args[7]).append(",").append(args[8]).toString()
				.split(",");
		if (utils.validateLabel(this.lblTransactionTab, "Transactions")) {
			if (utils.validateTableRowHeader(this.transactionsSummary_Tbl, colHdrs)) {
				System.out.println("Column Headers in UI matched with user data CDHP- Transactions Summary - sections");
				return true;
			}
		}
		return false;
	...
	/*
	 * @SCU #CommonMethodwithArgument:validateTransactionTableInCDHPAccountPlan
	 * #Description:This method selects the row for matching key-value pair in
	 * Transactions Tab for a particular 'Consumer Driven Health Plan Account Info'.
	 * #Arguments:CDHPAccountLevelTransactions-KeyValuePair Type:Table
	 * Keys:#Transaction Date#Service Start
	 * Date#Account#Claimant#Type#Description#Status#Transaction Amount
	 */
	public boolean validateTransactionTableInCDHPAccountPlan(String[] args) throws InterruptedException {
		utils.waitforpageload();
		utils.clickAnelemnt(this.lblTransactionTab, "ConsumerDrivenHealthPlan", "Transactions tab");
		for (String iterator : args) {
			String keyvaluepair = iterator;
		...
		if (utils.clickontablerowbasedonvalues(this.transactionsSummary_Tbl, args)) {
			System.out.println("Column Row matched with user data CDHP- Transactions Summary table");
			return true;
		} 
		return false;
	}
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateLblsInTransactionDetails
	 * #Description:This method validates the Labels/fields present in Transaction
	 * Details section. Type:TextBox
	 */
	public boolean validateLblsInTransactionDetails() {
		utils.waitforpageload();
		((JavascriptExecutor) Driver.getPgDriver()).executeScript("arguments[0].scrollIntoView(true);",
				this.lbl_TransactionDetails);
		ArrayList<String> fieldsNotPresent = new ArrayList<String>();
		try {
			if (this.lbl_TransactionDetails.isDisplayed()) {
				System.out.println("Transaction Details section Header is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Transaction tab - Transaction Details section Header is not displayed");
		...
		try {
			if (this.lbl_PlanYear.isDisplayed()) {
				System.out.println("Plan Year is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_MerchantProviderName.isDisplayed()) {
				System.out.println("Merchant/Provider Name is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_AccountType.isDisplayed()) {
				System.out.println("Account Type is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_AccountPostingDate.isDisplayed()) {
				System.out.println("Account Posting date is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Transaction tab - Account Posting date label is not displayed");
		...
		try {
			if (this.lbl_BalAftTransaction.isDisplayed()) {
				System.out.println("Balance After the Transaction is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Transaction tab - Balance After the Transaction label is not displayed");
		...
		try {
			if (this.lbl_MCC.isDisplayed()) {
				System.out.println("MCC is displayed");
			}
		} catch (Exception e) {
	...
	/*
	 * @SCU #CommonMethodwithArgument:validateTransactionDetails #Description:This
	 * method validates the fields present in Transaction Details section.
	 * #Arguments:TransactionDetails-KeyValuePair Type:Table
	...
	public boolean validateTransactionDetails(String[] args) {
		...
		for (String iterator : args) {
			if (!returnvar) {
				err.logcommonMethodError("validateTransactionDetails", "Check your " + keyvaluepair
						+ " Either Your input is wrong or the value found on application is incorrect");
				return false;
			...
			if (key.toLowerCase().contains("plan_year")) {
				...
				try {
					...
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Plan Year' CDHP - Transaction Details section");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_merchant_name.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Merchant/Provider Name' CDHP - Transaction Details section");
					return false;
				}
				continue;
				...
				hsa_type = value.trim();
				try {
					returnvar = this.txt_account_type.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Account Type' CDHP - Transaction Details section");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_account_postingdate.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Account Posting Date' CDHP - Transaction Details section");
					return false;
				}
				continue;
				...
				balance = value.trim();
				try {
					returnvar = this.txt_balance.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Balance After Transaction' CDHP - Transaction Details section");
					return false;
				}
				continue;
				...
				try {
					...
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'MCC' CDHP - Transaction Details section");
					return false;
				}
				continue;
			} else
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateLblsInAdjudicationDetails
	 * #Description:This method validates the Labels/fields present in Adjudication
	 * Details section. Type:TextBox
	 */
	...
	public boolean validateLblsInAdjudicationDetails() {
		...
		ArrayList<String> fieldsNotPresent = new ArrayList<String>();
		try {
			if (this.lbl_service_dates.isDisplayed()) {
				System.out.println("Service Dates is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Service Dates label is not displayed");
			System.out.println("Exception occured:Service Dates label is not displayed" + e);
			fieldsNotPresent.add("Service Dates");
		}

		try {
			if (this.lbl_service_type.isDisplayed()) {
				System.out.println("Service Type is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Service Type label is not displayed");
			System.out.println("Exception occured:Service Type label is not displayed" + e);
			fieldsNotPresent.add("Service Type");
		}

		try {
			if (this.lbl_service_details_desc.isDisplayed()) {
				System.out.println("Service Detail Desc is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Service Detail Desc label is not displayed");
		...
		try {
			if (this.lbl_pended.isDisplayed()) {
				System.out.println("Pended is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Pended label is not displayed");
			System.out.println("Exception occured:Pended label is not displayed" + e);
			fieldsNotPresent.add("Pended");
		}

		try {
			if (this.lbl_denied.isDisplayed()) {
				System.out.println("Denied is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Denied label is not displayed");
			System.out.println("Exception occured:Denied label is not displayed" + e);
			fieldsNotPresent.add("Denied");
		}

		try {
			if (this.lbl_excluded.isDisplayed()) {
				System.out.println("Excluded is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Excluded label is not displayed");
			System.out.println("Exception occured:Excluded label is not displayed" + e);
			fieldsNotPresent.add("Excluded");
		}

		try {
			if (this.lbl_applied_to_account_Deductible.isDisplayed()) {
				System.out.println("Applied to Account Deductible is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Applied to Account Deductible label is not displayed");
		...
		try {
			if (this.lbl_offset.isDisplayed()) {
				System.out.println("Offset is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Offset label is not displayed");
			System.out.println("Exception occured:Offset label is not displayed" + e);
			fieldsNotPresent.add("Offset");
		}

		try {
			if (this.lbl_low_funds.isDisplayed()) {
				System.out.println("Low Funds is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Low Funds label is not displayed");
			System.out.println("Exception occured:Low Funds label is not displayed" + e);
			fieldsNotPresent.add("Low Funds");
		}

		try {
			if (this.lbl_total_amount.isDisplayed()) {
				System.out.println("Total Amount is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Total Amount label is not displayed");
			System.out.println("Exception occured:Total Amount label is not displayed" + e);
			fieldsNotPresent.add("Total Amount");
		}

		try {
			if (this.lbl_eligible.isDisplayed()) {
				System.out.println("Eligible is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Eligible label is not displayed");
			System.out.println("Exception occured:Eligible label is not displayed" + e);
			fieldsNotPresent.add("Eligible");
		}

		try {
			if (this.lbl_Approved.isDisplayed()) {
				System.out.println("Approved is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Adjudication Details section -Approved label is not displayed");
	...
	/*
	 * @SCU #CommonMethodwithArgument:validateAdjudicationDetails #Description:This
	 * method validates the fields present in Adjudication Details section -in
	 * Transaction Tab. #Arguments:AdjudicationDetails-KeyValuePair Type:Table
	 * Keys:#service_dates#service_type#service_details_description#total_amount#
	 * pended#denied#excluded#eligible#applied_to_account_Deductible#offset#
	 * low_funds#approved
	 */
	public boolean validateAdjudicationDetails(String[] args) {
		...
		for (String iterator : args) {
			if (!returnvar) {
				err.logcommonMethodError("validateAdjudicationDetails", "Check your " + keyvaluepair
						+ " Either Your input is wrong or the value found on application is incorrect");
				return false;
			...
			if (key.toLowerCase().contains("service_dates")) {
				try {
					returnvar = this.txt_service_dates.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Service Dates' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("service_type")) {
				try {
					returnvar = this.txt_service_type.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Service Type' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("service_details_description")) {
				try {
					returnvar = this.txt_service_details_desc.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Service Details Description' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("total_amount")) {
				try {
					returnvar = this.txt_total_amount.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Total Amount' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("pended")) {
				try {
					returnvar = this.txt_pended.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Pended' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("denied")) {
				try {
					returnvar = this.txt_denied.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Denied' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("excluded")) {
				try {
					returnvar = this.txt_excluded.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Excluded' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("eligible")) {
				try {
					returnvar = this.txt_eligible.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Eligible' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("applied_to_account_deductible")) {
				try {
					returnvar = this.txt_applied_to_account_Deductible.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Applied to account Deductible' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("offset")) {
				try {
					returnvar = this.txt_offset.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Offset' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("low_funds")) {
				try {
					returnvar = this.txt_low_funds.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Low Funds' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("approved")) {
				try {
					returnvar = this.txt_Approved.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Approved' Adjudication Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateLblsInClaimComments
	 * #Description:This method validates the Labels/fields present in Claim
	 * Comments section. Type:TextBox
	 */
	...
	public boolean validateLblsInClaimComments() {
		utils.waitForElementToBeVisible(tablehdrClaimComments);
		((JavascriptExecutor) Driver.getPgDriver()).executeScript("arguments[0].scrollIntoView(true);",
				this.tablehdrClaimComments);
		utils.clickAnelemnt(this.tablehdrClaimComments, "ConsumerDrivenHealthPlan", "Claim Comments");
		ArrayList<String> fieldsNotPresent = new ArrayList<String>();
		try {
			if (this.lbl_denial_reason.isDisplayed()) {
				System.out.println("Denial Reason is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Claim Comments section -Denial Reason label is not displayed");
			System.out.println("Exception occured:Denial Reason label is not displayed" + e);
			fieldsNotPresent.add("Denial Reason");
		}

		try {
			if (this.lbl_denial_comment.isDisplayed()) {
				System.out.println("Denied Comment is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Claim Comments section -Denied Comment label is not displayed");
			System.out.println("Exception occured:Denied Comment label is not displayed" + e);
			fieldsNotPresent.add("Denied Comment");
		}

		try {
			if (this.lbl_pended_reason.isDisplayed()) {
				System.out.println("Pended Reason is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Claim Comments section -Pended Reason label is not displayed");
			System.out.println("Exception occured:Pended Reason label is not displayed" + e);
			fieldsNotPresent.add("Pended Reason");
		}

		try {
			if (this.lbl_pended_comment.isDisplayed()) {
				System.out.println("Pended Comment is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Claim Comments section -Pended Comment label is not displayed");
	...
	/*
	 * @SCU #CommonMethodwithArgument:validateClaimComments #Description:This method
	 * validates the fields present in Claim Comments section -in Transaction Tab.
	 * #Arguments:ClaimComments-KeyValuePair Type:Table
	...
	public boolean validateClaimComments(String[] args) {
		...
		for (String iterator : args) {
			if (!returnvar) {
				err.logcommonMethodError("validateClaimComments", "Check your " + keyvaluepair
						+ " Either Your input is wrong or the value found on application is incorrect");
				return false;
			...
			if (key.toLowerCase().contains("denial_reason")) {
				try {
					returnvar = this.txt_denial_reason.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Denial Reason' Claim Comments section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("denial_comment")) {
				try {
					returnvar = this.txt_denial_comment.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Denial Comment' Claim Comments section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("pended_reason")) {
				try {
					returnvar = this.txt_pended_reason.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Pended Reason' Claim Comments section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("pended_comment")) {
				try {
					returnvar = this.txt_pended_comment.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Pended Comment' Claim Comments section -in Transaction Tab");
					return false;
				}
				continue;
			} else
				err.logcommonMethodError("ConsumerDrivenHealthPlan",
						"Check your key in yourkeypair. Make sure you are following the same pattern for input - validateClaimComments");
			return false;
		}
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateLblsInReimbursementDetails
	 * #Description:This method validates the Labels/fields present in Reimbursement
	 * Details section. Type:TextBox
	 */
	...
	public boolean validateLblsInReimbursementDetails() {
		...
		ArrayList<String> fieldsNotPresent = new ArrayList<String>();
		try {
			if (this.lbl_reimb_date.isDisplayed()) {
				System.out.println("Reimbursement Date is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Reimbursement Details section -Reimbursement Date label is not displayed");
			System.out.println("Exception occured:Reimbursement Date label is not displayed" + e);
			fieldsNotPresent.add("Reimbursement Date");
		}

		try {
			if (this.lbl_reimb_method.isDisplayed()) {
				System.out.println("Reimbursement Method is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Reimbursement Details section -Reimbursement Method label is not displayed");
		...
		try {
			if (this.lbl_amount_reimb.isDisplayed()) {
				System.out.println("Amount Reimbursed is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Reimbursement Details section -Amount Reimbursed label is not displayed");
			System.out.println("Exception occured: Amount Reimbursed label is not displayed" + e);
			fieldsNotPresent.add("Amount Reimbursed");
		}

		try {
			if (this.lbl_reissued_check.isDisplayed()) {
				System.out.println("Reissued Check is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Reimbursement Details section -Reissued Check label is not displayed");
			System.out.println("Exception occured:Reissued Check label is not displayed" + e);
			fieldsNotPresent.add("Reissued Check");
		}

		try {
			if (this.lbl_check_trace_no.isDisplayed()) {
				System.out.println("Check / Trace Number is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Reimbursement Details section -Check / Trace Number label is not displayed");
	...
	/*
	 * @SCU #CommonMethodwithArgument:validateReimbursementDetails #Description:This
	 * method validates the fields present in Reimbursement Details section -in
	 * Transaction Tab. #Arguments:ReimbursementDetails-KeyValuePair Type:Table
	...
	public boolean validateReimbursementDetails(String[] args) {
		...
		for (String iterator : args) {
			if (!returnvar) {
				err.logcommonMethodError("validateReimbursementDetails", "Check your " + keyvaluepair
						+ " Either Your input is wrong or the value found on application is incorrect");
				return false;
			...
			if (key.toLowerCase().contains("reimb_date")) {
				try {
					...
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Reimbursement Date' Reimbursement Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("reimb_method")) {
				try {
					returnvar = this.txt_reimb_method.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Reimbursement Method' Reimbursement Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("amount_reimb")) {
				try {
					returnvar = this.txt_amount_reimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Amount Reimbursed' Reimbursement Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("reissued_check")) {
				try {
					returnvar = this.txt_reissued_check.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Reissued Check' Reimbursement Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("check_trace_no")) {
				try {
					returnvar = this.txt_check_trace_no.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Check / Trace Number' Reimbursement Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else
				err.logcommonMethodError("ConsumerDrivenHealthPlan",
						"Check your key in yourkeypair. Make sure you are following the same pattern for input - Reimbursement Details");
			return false;
		}

		if (returnvar) {
			System.out.println(
					"ConsumerDrivenHealthPlan - validated Reimbursement Details tab info checked Successfully");
			return true;
		} else {
			int tot_i = args.length;
			err.logcommonMethodError("ConsumerDrivenHealthPlan",
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateLblsInProviderPaymentDetails
	 * #Description:This method validates the Labels/fields present in Provider
	 * Payment Details section. Type:TextBox
	 */
	...
	public boolean validateLblsInProviderPaymentDetails() {
		utils.waitForElementToBeVisible(tableProviderPaymentDetails);
		((JavascriptExecutor) Driver.getPgDriver()).executeScript("arguments[0].scrollIntoView(true);",
				this.tableProviderPaymentDetails);
		utils.clickAnelemnt(this.tableProviderPaymentDetails, "ConsumerDrivenHealthPlan", "Provider Payment Details");
		ArrayList<String> fieldsNotPresent = new ArrayList<String>();
		try {
			if (this.lbl_provider_name.isDisplayed()) {
				System.out.println("Provider Name is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Provider Payment Details section -Provider Name label is not displayed");
			System.out.println("Exception occured:Provider Name label is not displayed" + e);
			fieldsNotPresent.add("Provider Name");
		}

		try {
			if (this.lbl_address1.isDisplayed()) {
				System.out.println("Address 1 is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Provider Payment Details section -Address 1 label is not displayed");
			System.out.println("Exception occured:Address 1 label is not displayed" + e);
			fieldsNotPresent.add("Address 1");
		}

		try {
			if (this.lbl_address2.isDisplayed()) {
				System.out.println("Address 2 is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Provider Payment Details section -Address 2 label is not displayed");
			System.out.println("Exception occured:Address 2 label is not displayed" + e);
			fieldsNotPresent.add("Address 2");
		}

		try {
			if (this.lbl_city.isDisplayed()) {
				System.out.println("City is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Provider Payment Details section -City label is not displayed");
			System.out.println("Exception occured:City label is not displayed" + e);
			fieldsNotPresent.add("City");
		}

		try {
			if (this.lbl_state.isDisplayed()) {
				System.out.println("State is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Provider Payment Details section -State label is not displayed");
			System.out.println("Exception occured:State label is not displayed" + e);
			fieldsNotPresent.add("State");
		}

		try {
			if (this.lbl_zipcode.isDisplayed()) {
				System.out.println("Zipcode is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP-Provider Payment Details section -Zipcode label is not displayed");
	...
	/*
	 * @SCU #CommonMethodwithArgument:validateProviderPaymentDetails
	 * #Description:This method validates the fields present in Provider Payment
	 * Details section -in Transaction Tab.
	 * #Arguments:ProviderPaymentDetails-KeyValuePair Type:Table
	...
	public boolean validateProviderPaymentDetails(String[] args) {
		...
		for (String iterator : args) {
			if (!returnvar) {
				err.logcommonMethodError("validateProviderPaymentDetails", "Check your " + keyvaluepair
						+ " Either Your input is wrong or the value found on application is incorrect");
				return false;
			...
			if (key.toLowerCase().contains("provider_name")) {
				try {
					returnvar = this.txt_provider_name.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Provider Name' Provider Payment Details section -in Transaction Tab");
					return false;
				}
				continue;
				...
				try {
					...
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Address1' Provider Payment Details section -in Transaction Tab");
					return false;
				}
				continue;
				...
				try {
					...
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Address2' Provider Payment Details section -in Transaction Tab");
					return false;
				}
				continue;
				...
				try {
					...
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'City' Provider Payment Details section -in Transaction Tab");
					return false;
				}
				continue;
				...
				try {
					...
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'State' Provider Payment Details section -in Transaction Tab");
					return false;
				}
				continue;
				...
				try {
					...
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Zipcode' Provider Payment Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateLblsInClaimLineDetails
	 * #Description:This method validates the Labels/fields present in Claim Line
	 * Details section. Type:TextBox
	 */
	public boolean validateLblsInClaimLineDetails() {
		((JavascriptExecutor) Driver.getPgDriver()).executeScript("arguments[0].scrollIntoView(true);",
				this.hdrClaimLineDetails);
		String args[] = { "Billed Amount", "Allowed Amount", "Insurance Paid Amount", "Deductible", "Coinsurance",
				"CoPay", "Benefit Max Met", "Referenced Based Benefit Excess", "Above Reasonable and Customary",
		"Not Covered" };
		if(utils.clickAnelemnt(this.hdrClaimLineDetails, "ConsumerDrivenHealthPlan", "Claim Line Details"))
			return validateRowValuesInClaimTable(this.claimLineDetailsTbl, args);
		return false;
	}

	/*
	 * @SCU #CommonMethodwithArgument:validateClaimLineDetails #Description:This
	 * method validates the fields present in Claim Line Details section -in
	 * Transaction Tab. #Arguments:ClaimLineDetails-KeyValuePair Type:Table
	 * Keys:#billed_amt_request#billed_amt_reimb#allowed_amt_request#
	 * allowed_amt_reimb#ins_paid_amt_request#ins_paid_amt_reimb#deduct_amt_request#
	 * deduct_amt_reimb#coIns_amt_request#coIns_amt_reimb#copay_amt_request#
	 * copay_amt_reimb#bene_met_maxrequest#bene_met_maxreimb#rbb_excess_request#
	 * rbb_excess_reimb#above_RC_request#above_RC_reimb#notcovered_amt_request#
	 * notcovered_amt_reimb
	 */
	public boolean validateClaimLineDetails(String[] args) {
		...
		for (String iterator : args) {
			if (!returnvar) {
				err.logcommonMethodError("validateClaimLineDetails", "Check your " + keyvaluepair
						+ " Either Your input is wrong or the value found on application is incorrect");
				return false;
			...
			if (key.toLowerCase().contains("billed_amt_request")) {
				try {
					returnvar = this.txt_billed_amt_request.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Billed Amount Requested' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("billed_amt_reimb")) {
				try {
					returnvar = this.txt_billed_amt_reimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Billed Amount Reimbursed' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("allowed_amt_request")) {
				try {
					returnvar = this.txt_allowed_amt_request.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Allowed Amount Requested' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("allowed_amt_reimb")) {
				try {
					returnvar = this.txt_allowed_amt_reimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Allowed Amount Reimbursed' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("ins_paid_amt_request")) {
				try {
					returnvar = this.txt_ins_paid_amt_request.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Insurance Paid Amount Requested' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("ins_paid_amt_reimb")) {
				try {
					returnvar = this.txt_ins_paid_amt_reimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Insurance Paid Amount Reimbursed' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("deduct_amt_request")) {
				try {
					returnvar = this.txt_deduct_amt_request.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Deductible Amount Requested' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("deduct_amt_reimb")) {
				try {
					returnvar = this.txt_deduct_amt_reimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Deductible Amount Reimbursed' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("coins_amt_request")) {
				try {
					returnvar = this.txt_coIns_amt_request.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'CoInsurance Amount Requested' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("coins_amt_reimb")) {
				try {
					returnvar = this.txt_coIns_amt_reimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'CoInsurance Amount Reimbursed' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("copay_amt_request")) {
				try {
					returnvar = this.txt_copay_amt_request.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Copay Amount Requested' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("copay_amt_reimb")) {
				try {
					returnvar = this.txt_copay_amt_reimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Copay Amount Reimbursed' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("bene_met_maxrequest")) {
				try {
					returnvar = this.txt_bene_met_maxrequest.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Benefits Met Max Requested' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("bene_met_maxreimb")) {
				try {
					returnvar = this.txt_bene_met_maxreimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Benefits Met Max Reimbursed' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("rbb_excess_request")) {
				try {
					returnvar = this.txt_rbb_excess_request.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'RBB Excess Requested' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("rbb_excess_reimb")) {
				try {
					returnvar = this.rbb_excess_reimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'RBB Excess Reimbursed' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("above_rc_request")) {
				try {
					returnvar = this.txt_above_RC_request.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Above R & C Requested' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("above_rc_reimb")) {
				try {
					returnvar = this.txt_above_RC_reimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Above R & C Reimbursed' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("notcovered_amt_request")) {
				try {
					returnvar = this.txt_notcovered_amt_request.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Not Covered Amount Requested' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else if (key.toLowerCase().contains("notcovered_amt_reimb")) {
				try {
					returnvar = this.txt_notcovered_amt_reimb.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Not Covered Amount Reimbursed' Claim Line Details section -in Transaction Tab");
					return false;
				}
				continue;
			} else
	...
	/*
	 * @SCU #CommonMethodwithArgument:validateCustodianBank #Description:This method
	 * validates the fields present in Custodian Bank section.
	 * #Arguments:CustodianBank-KeyValuePair Type:Table
	 * Keys:#hsaAcctNo#hsaBankName#tot_cont_ytd#empl_cont_ytd#
	 * total_Contributed_Current_yr#reportable_HSA_Contributions#
	 * employee_Contributions_Prior_yr#employer_Contributions_Prior_yr#
	 * total_Contributions_Prior_yr#reportable_HS_Contributions_Prior_yr#
	 * distributions_Current_yr#distributions_Prior_yr#investmentBalance#
	 * thresholdtoInvest#amount_Available_to_Invest
	 */
	public boolean validateCustodianBank(String[] args) {
		...
		for (String iterator : args) {
			if (!returnvar) {
				err.logcommonMethodError("validateCustodianBank", "Check your " + keyvaluepair
						+ " Either Your input is wrong or the value found on application is incorrect");
				return false;
			...
			if (key.equalsIgnoreCase("hsaAcctNo")) {
				try {
					...
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'HSA Account Number' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_hsaBankName.getText().toLowerCase().trim()
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'HSA Bank Name' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
			} else if (key.equalsIgnoreCase("tot_cont_ytd")) {
				try {
					returnvar = this.txt_tot_cont_ytd.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Total Contributions YTD' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
			} else if (key.equalsIgnoreCase("empl_cont_ytd")) {
				try {
					returnvar = this.txt_empl_cont_ytd.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Employer Contributions YTD' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
			} else if (key.equalsIgnoreCase("total_Contributed_Current_yr")) {
				try {
					returnvar = this.txt_Total_Contributed_Current_Year.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Total Contributed Current Year' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_Reportable_HSA_Contributions.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Reportable HSA Contributions' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_Employee_Contributions_Prior_Year.getText().toLowerCase().trim()
							.replace(",", "").contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Employee Contributions Prior Year' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_Employer_Contributions_Prior_Year.getText().toLowerCase().trim()
							.replace(",", "").contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Employer Contributions Prior Year' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_Total_Contributions_Prior_Year.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Total Contributions Prior Year' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_Reportable_HS_Contributions_Prior_Year.getText().toLowerCase().trim()
							.replace(",", "").contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Reportable HSA Contributions Prior Year' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_Distributions_Current_Year.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Distributions Current Year' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_Distributions_Prior_Year.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Distributions Prior Year' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_InvestmentBalance.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Investment Balance' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_ThresholdtoInvest.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Threshold to Invest' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
				...
				try {
					returnvar = this.txt_Amount_Available_to_Invest.getText().toLowerCase().trim().replace(",", "")
							.contains(value.toLowerCase().trim());
				} catch (Exception e) {
					err.logError("ConsumerDrivenHealthPlan",
							"Exception occured while retrieving 'Amount Available to Invest' CDHP - Custodian Bank Tab");
					return false;
				}
				continue;
			} else
				err.logcommonMethodError("ConsumerDrivenHealthPlan",
						"Check your key in yourkeypair. Make sure you are following the same pattern for input - CustodianBank");
			return false;
		}
	...
	/*
	 * @SCU #CommonMethodwithArgument:verifyCardInformationHeaders #Description:This
	 * method verifies the fields and headers present in Card Information table.
	 * #Arguments:Card Info headers
	 * -"First Name,Last Name,Card # - Last 4 Digits,Status,Mailed Date"
	 * Type:TextBox
	 */
	public boolean verifyCardInformationHeaders(String[] args) {
		...
		String space = " ";
		StringBuilder input = new StringBuilder();
		System.out.println("Newly constructed Input:"
		...
		String colHdrs[] = input.append(space).append(",").append(args[0]).append(",").append(args[1]).append(",")
				.append(args[2]).append(",").append(args[3]).append(",").append(args[4]).toString().split(",");
		try {
			if (this.lblCardInfoDiscussedWithContact.isDisplayed() && this.reviewChkBox.isDisplayed()) {
				System.out.println("'Cards information discussed with contact' is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -CARD Info tab - 'Cards information discussed with contact' is displayed' label is not displayed");
			System.out.println(" 'Cards information discussed with contact' is displayed" + e);
		}

		return utils.validateTableRowHeader(this.cardInformation_Tbl, colHdrs);

	}

	/*
	 * @SCU #CommonMethodwithArgument:validateCardInformationInCDHP
	 * #Description:This method selects the row for matching key-value pair in Card
	 * Information Tab for a particular 'Consumer Driven Health Plan Account Info'.
	 * #Arguments:CardInformation-KeyValuePair Type:Table Keys:#First Name#Last
	 * Name#Card#Status#Mailed Date
	 */
	public boolean validateCardInformationInCDHP(String[] args) throws InterruptedException {
		...
		return utils.clickontablerowbasedonvalues(this.cardInformation_Tbl, args);
	}
	...
	/*
	 * @SCU #CommonMethodwithArgument:cancelCDHPTask #Description:This method is
	 * used to cancel CDHP Task and verify request status 'Resolved- Cancelled' is
	 * through Case-search. #Arguments:Cancellation Reason Type:Dropdown
	...
	public boolean cancelCDHPTask(String[] args) throws InterruptedException {
		...
		if(utils.clickAnelemnt(this.btnOtherActions, "ConsumerDrivenHealthPlan", "Other Actions"))
			if(utils.clickAnelemnt(this.lnkOtherCancelThisWork, "ConsumerDrivenHealthPlan", "Cancel this work"))
				if (utils.validateHeader(this.sHeaderCDHP, "Cancel this work")) 
					if(utils.selectDropDownbyVisibleString(this.drpdwnCancellationreason, args[0], "ConsumerDrivenHealthPlan",
							"Cancellation Reason"))
		...
		/*utils.waitforpageload();
			...
			if (utils.clickAnelemnt(this.searchResults, "ConsumerDrivenHealthPlan", "Results Hyperlink")) {
				Thread.sleep(4000);
				return !utils.isProxyWebelement(cancelStatus);
			} 
		}*/
		return false;
	}
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateSectionsDisplayedInCustodianBankTab
	 * #Description:This method validated the labels and sections displayed in
	 * Custodian Bank Tab for a particular 'Consumer Driven Health Plan Account
	 * Info'. Type:TextBox
	 */
	public boolean validateSectionsDisplayedInCustodianBankTab() {
		...
		try {
			...
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - 'Custodian Bank information discussed with contact' label is not displayed");
		...
		try {
			if (this.lbl_hsaAcctNo.isDisplayed()) {
				System.out.println("'HSA Account Number' is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - 'HSA Account Number' label is not displayed");
			System.out.println(" 'HSA Account Number' label is not displayed" + e);
			fieldsNotPresent.add("HSA Account Number");
		}

		try {
			if (this.lbl_hsaBankName.isDisplayed()) {
				System.out.println("'HSA Bank Name' is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - 'HSA Bank Name' label is not displayed");
			System.out.println(" 'HSA Bank Name' label is not displayed" + e);
			fieldsNotPresent.add("HSA Bank Name");
		}

		try {
			if (this.lblContributionsSummary.isDisplayed()) {
				System.out.println("'Contributions Summary' is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - 'Contributions Summary' label is not displayed");
			System.out.println(" 'Contributions Summary' label is not displayed" + e);
			fieldsNotPresent.add("Contributions Summary");
		}

		try {
			if (this.lbl_tot_cont_ytd.isDisplayed()) {
				System.out.println("Total Contributions YTD: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Total Contributions YTD label is not displayed");
			System.out.println(" 'Total Contributions YTD' label is not displayed" + e);
			fieldsNotPresent.add("Total Contributions YTD");
		}

		try {
			if (this.lbl_empl_cont_ytd.isDisplayed()) {
				System.out.println("Employer Contributions YTD: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Employer Contributions YTD: label is not displayed");
			System.out.println("Employer Contributions YTD label is not displayed" + e);
			fieldsNotPresent.add("Employer Contributions YTD");
		}

		try {
			if (this.lbl_Total_Contributed_Current_Year.isDisplayed()) {
				System.out.println("Total Contributed Current Year: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Total Contributed Current Year: label is not displayed");
		...
		try {
			if (this.lbl_Reportable_HSA_Contributions.isDisplayed()) {
				System.out.println("Reportable HSA Contributions: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Reportable HSA Contributions: label is not displayed");
		...
		try {
			if (this.lbl_Employee_Contributions_Prior_Year.isDisplayed()) {
				System.out.println("Employee Contributions Prior Year: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Employee Contributions Prior Year: label is not displayed");
		...
		try {
			if (this.lbl_Employer_Contributions_Prior_Year.isDisplayed()) {
				System.out.println("Employer Contributions Prior Year: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Employer Contributions Prior Year: label is not displayed");
		...
		try {
			if (this.lbl_Total_Contributions_Prior_Year.isDisplayed()) {
				System.out.println("Total Contributions Prior Year: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Total Contributions Prior Year: label is not displayed");
		...
		try {
			if (this.lbl_Reportable_HS_Contributions_Prior_Year.isDisplayed()) {
				System.out.println("Reportable HSA Contributions Prior Year: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Reportable HSA Contributions Prior Year: label is not displayed");
		...
		try {
			if (this.lblContributionsSummary.isDisplayed()) {
				System.out.println("'Distributions Summary' is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - 'Distributions Summary' label is not displayed");
			System.out.println(" 'Distributions Summary' label is not displayed" + e);
			fieldsNotPresent.add("Distributions Summary");
		}

		try {
			if (this.lbl_Distributions_Current_Year.isDisplayed()) {
				System.out.println("Distributions Current Year: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Distributions Current Year: label is not displayed");
			System.out.println("Distributions Current Year label is not displayed" + e);
			fieldsNotPresent.add("Distributions Current Year");
		}

		try {
			if (this.lbl_Distributions_Prior_Year.isDisplayed()) {
				System.out.println("Distributions Prior Year: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab -Distributions Prior Year : label is not displayed");
		...
		try {
			if (this.lblContributionsSummary.isDisplayed()) {
				System.out.println("'Investment Summary' is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - 'Investment Summary' label is not displayed");
			System.out.println(" 'Investment Summary' label is not displayed" + e);
			fieldsNotPresent.add("Investment Summary");
		}

		try {
			if (this.lbl_InvestmentBalance.isDisplayed()) {
				System.out.println("Investment Balance: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Investment Balance: label is not displayed");
			System.out.println("Investment Balance label is not displayed" + e);
			fieldsNotPresent.add("Investment Balance");
		}

		try {
			if (this.lbl_ThresholdtoInvest.isDisplayed()) {
				System.out.println("Threshold to Invest: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Threshold to Invest: label is not displayed");
			System.out.println("Threshold to Invest label is not displayed" + e);
			fieldsNotPresent.add("Threshold to Invest");
		}

		try {
			if (this.lbl_Amount_Available_to_Invest.isDisplayed()) {
				System.out.println("Amount Available to Invest: is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Custodian Bank tab - Amount Available to Invest: label is not displayed");
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:verifyLitesLinkDisplayForLumenousCDHPAcct
	 * #Description:This method verifies if Lites link is displayed for a Lumenous
	 * CDHP Account Type:TextBox
	 */
	public boolean verifyLitesLinkDisplayForLumenousCDHPAcct() {
		utils.waitforpageload();
		if (utils.validateHeader(this.sHeaderCDHP, "View Consumer Driven Health Plan Accounts")) {
			return !utils.isProxyWebelement(litesLink);
		}
		return false;
	}

	/*
	 * @SCU #CommonMethodwithoutArgument:verifyNoLitesLinkDisplay #Description:This
	 * method verifies if No Lites link is displayed for a CDHP Account Type:TextBox
	 */
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:clickOnLitesLinkForLumenousCDHPAcct
	 * #Description:In this method,Lites link is clicked and we verify Member view
	 * is populated in default when SSO is implemented Type:TextBox
	 */
	public boolean clickOnLitesLinkForLumenousCDHPAcct() throws InterruptedException {
		utils.clickAnelemnt(litesLink, "ConsumerDrivenHealthPlan", "Lites Link");
		...
		if (title.contains("Search")) {
			return !utils.isProxyWebelement(memberViewText);
		}
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:verifyMemberWithNonAnthemNonLumenousAccount
	 * #Description:This method validates if appropriate message is displayed when
	 * Member has no active Anthem CDHP or Lumenous CDHP enrollments Type:TextBox
	 */
	public boolean verifyMemberWithNonAnthemNonLumenousAccount() {
		...
		try {
			...
			String msgTxt1 = "This member doesn’t have Anthem CDH or Lumenous as of " + month + " "
					+ today.getDayOfMonth() + ", " + today.getYear() + ", "
					+ "go to the vendor information section on the contract tab";
			String msgTxt2 = "under the member composite to find to see if the member has another CDHP vendor to refer the member to.";
			if (msg.get(0).getText().trim().replace("0", "").equalsIgnoreCase(msgTxt1.replace("0", ""))
					&& msg.get(1).getText().trim().equalsIgnoreCase(msgTxt2)) {
			...
			err.logError("verifyMemberWithNonAnthemNonLumenousAccount",
					"This member does not have an Anthem CDH or Lumenous account as of 'current date' is not displayed");
			return false;
		}
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:verifyMemberWithNonAnthemLumenousAccount
	 * #Description:This method validates if appropriate message is displayed when
	 * Member has no active Anthem account but Lumenous CDHP enrollments.
	 * Type:TextBox
	 */
	public boolean verifyMemberWithNonAnthemLumenousAccount() {
		...
		DateTimeFormatter f = DateTimeFormatter.ofPattern("MMM");
		String month = today.format(f);
		String expectedMsg = "This member does not have Anthem CDH account as of " + month + " " + today.getDayOfMonth()
		+ ", " + today.getYear() + ". "
		+ "For Lumenous CDHP information, click on the below link to access the corresponding application.";
		utils.waitforpageload();
		String msgText1 = Driver.pgDriver.findElement(By.xpath("//span/i[starts-with(text(),'This member')]")).getText()
				.trim() + " " + month + " " + today.getDayOfMonth() + ", " + today.getYear() + ". ";
		String msgText2 = Driver.pgDriver.findElement(By.xpath("//em/span")).getText().trim().replace("<br>", "")
				.replace("\n", " ");
		String actual = msgText1.concat(msgText2);
		return utils.isvalueMatch_compareto(actual, expectedMsg);
	}

	/*
	 * @SCU #CommonMethodwithoutArgument:verifyMemberWithAnthemLumenousAccount
	 * #Description:This method verifies that no message is displayed when Member
	 * has active Anthem CDHP or Lumenous CDHP enrollments Type:TextBox
	 */
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:verifyDisplayofAnthemCDHlink #Description:
	 * This method verifies if 'AnthemCDH' link is displayed on the "View CDHP" page
	 * and user clicks on it - to navigate to the WCA SSO Consumer Service.
	 * Type:TextBox
	 */
	public boolean verifyDisplayofAnthemCDHlink() {
		if (utils.validateHeader(this.sHeaderCDHP, "View Consumer Driven Health Plan Accounts")) {
			try {
				if (this.anthemCDHLink.isDisplayed()) {
					utils.clickAnelemnt(this.anthemCDHLink, "ConsumerDrivenHealthPlan", "Anthem CDH");
					...
					if (title.contains("WCA SSO Consumer Service")) {
						...
						err.logcommonMethodError("ConsumerDrivenHealthPlan",
								"Error in switching to childwindow -Anthem CDH");
						return false;
					}
				}
			} catch (Exception e) {
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateLblsInCardTransactionDetails
	 * #Description:This method validates labels displayed in Card Transaction
	 * Details section. Type:TextBox
	 */
	public boolean validateLblsInCardTransactionDetails() {
		((JavascriptExecutor) Driver.getPgDriver()).executeScript("arguments[0].scrollIntoView(true);",
				this.lbl_CardTransactionDetails);
		utils.clickAnelemnt(this.lbl_CardTransactionDetails, "CDHP", "Card Transaction Details");
		ArrayList<String> fieldsNotPresent = new ArrayList<String>();
		try {
			if (this.lbl_CardTransactionDetails.isDisplayed()) {
				System.out.println("Card Transaction Details section Header is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Transaction tab - Card Transaction Details section Header is not displayed");
		...
		try {
			if (this.lbl_SettlementDate.isDisplayed()) {
				System.out.println("Settlement Date is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_MerchantID.isDisplayed()) {
				System.out.println("Merchant ID is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_TerminalID.isDisplayed()) {
				System.out.println("Terminal ID is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_TerminalName.isDisplayed()) {
				System.out.println("Terminal Name is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_TerminalCity.isDisplayed()) {
				System.out.println("Terminal City is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_TerminalState.isDisplayed()) {
				System.out.println("Terminal State is displayed");
			}
		} catch (Exception e) {
		...
		try {
			if (this.lbl_TotalTransactionAmount.isDisplayed()) {
				System.out.println("Total Transaction Amount is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Transaction tab - Total Transaction Amount label is not displayed");
		...
		try {
			if (this.lbl_IneligibleAmount.isDisplayed()) {
				System.out.println("Ineligible Amount is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Transaction tab - Ineligible Amount label is not displayed");
			System.out.println("Exception occured:Ineligible Amount label is not displayed" + e);
			fieldsNotPresent.add("Ineligible Amount");
		}

		try {
			if (this.lbl_IneligibleReason.isDisplayed()) {
				System.out.println("Ineligible Reason is displayed");
			}
		} catch (Exception e) {
			err.logError("ConsumerDrivenHealthPlan",
					"CDHP -Transaction tab - Ineligible Reason label is not displayed");
	...
	/*
	 * @SCU #CommonMethodwithArgument:validateCardTransactionDetails
	 * #Description:This method validates values displayed in Card Transaction
	 * Details section. #Arguments:CDHPCardTransactionDetails-KeyValuePair
	 * Type:Table Keys:Settlement Date#Merchant ID#Terminal ID#Terminal
	 * Name#Terminal City#Terminal State#Total Transaction Amount#Ineligible
	 * Amount#Ineligible Reason
	 */
	public boolean validateCardTransactionDetails(String[] args) {
		...
		if (utils.validateLabel(this.lbl_CardTransactionDetails, "Card Transaction Details")) {
			...
			for (String iterator : args) {
				if (!returnvar) {
					err.logcommonMethodError("validateCardTransactionDetails", "Check your " + keyvaluepair
							+ " Either Your input is wrong or the value found on application is incorrect");
					return false;
				...
				if (key.equalsIgnoreCase("Settlement Date")) {
					try {
						returnvar = this.txt_SettlementDate.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Settlement Date' in - Card Transaction Details Tab");
						return false;
					}
					continue;
				} else if (key.equalsIgnoreCase("Merchant ID")) {
					try {
						returnvar = this.txt_MerchantID.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Merchant ID' in - Card Transaction Details Tab");
						return false;
					}
					continue;
				} else if (key.equalsIgnoreCase("Terminal ID")) {
					try {
						returnvar = this.txt_TerminalID.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Terminal ID' in - Card Transaction Details Tab");
						return false;
					}
					continue;
				} else if (key.equalsIgnoreCase("Terminal Name")) {
					try {
						returnvar = this.txt_TerminalName.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Terminal Name' in - Card Transaction Details Tab");
						return false;
					}
					continue;
				} else if (key.equalsIgnoreCase("Terminal City")) {
					try {
						returnvar = this.txt_TerminalCity.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Terminal City' in - Card Transaction Details Tab");
						return false;
					}
					continue;
				} else if (key.equalsIgnoreCase("Terminal State")) {
					try {
						returnvar = this.txt_TerminalState.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Terminal State' in - Card Transaction Details Tab");
						return false;
					}
					continue;
				} else if (key.equalsIgnoreCase("Total Transaction Amount")) {
					try {
						returnvar = this.txt_TotalTransactionAmount.getText().toLowerCase().trim().replace(",", "")
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Total Transaction Amount' in - Card Transaction Details Tab");
						return false;
					}
					continue;
				} else if (key.equalsIgnoreCase("Ineligible Amount")) {
					try {
						returnvar = this.txt_IneligibleAmount.getText().toLowerCase().trim().replace(",", "")
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Ineligible Amount' in - Card Transaction Details Tab");
						return false;
					}
					continue;
				} else if (key.equalsIgnoreCase("Ineligible Reason")) {
					try {
						returnvar = this.txt_IneligibleReason.getText().toLowerCase().trim()
								.contains(value.toLowerCase().trim());
					} catch (Exception e) {
						err.logError("ConsumerDrivenHealthPlan",
								"Exception occured while retrieving 'Ineligible Reason' in - Card Transaction Details Tab");
						return false;
					}
					continue;
				} else
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateExternalSearchPageIsDisplayedForISG
	 * #Description:This method validates if External Search page is launched for
	 * Individual and Small Group Business, while adding CDHP Task. Type:TextBox
	 */
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateToBeTaggedMsgOnSubmit
	 * #Description:This method validates the message displayed, when user clicks on
	 * submit without selecting a CDHP Account. Type:TextBox
	 */
	public boolean validateToBeTaggedMsgOnSubmit() {
		utils.waitforpageload();
		utils.clickAnelemnt(this.btnSubmit, "ConsumerDrivenHealthPlan", "Submit");
		System.out.println("Submit clicked successfully - on CDHP page");
		Driver.pgDriver.switchTo().alert().accept();
		System.out.println("No CDHP accounts are tagged.  Do you want to Submit?! - on CDHP page");
		return true;
	}

	/*
	 * @SCU #CommonMethodwithArgument:selectCDHPAccount #Description:This method
	 * selects a particular 'Consumer Driven Health Plan Account', depending on the
	 * parameters passed for selection. #Arguments:CDHPAccount-KeyValuePair
	 * Type:Table Keys:#Account Description#Status#Effective Date#Plan Start
	 * Date#Plan End Date
	 */
	...
	/*
	 * @SCU #CommonMethodwithArgument:selectTransactionInCDHPAccount
	 * #Description:This method selects a particular transaction in 'CDHP Account',
	 * depending on the parameters passed for selection.
	 * #Arguments:Transaction-KeyValuePair Type:Table Keys:#Transaction Date#Service
	 * Start Date#Account#Claimant#Type#Description#Status#Transaction Amount
	 */
	public boolean selectTransactionInCDHPAccount(String[] args) {
		try {
			...
			transaction.findElement(By.tagName("td")).click();
			utils.waitforpageload();
		} catch (Exception e) {
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:checkSummaryInformationDiscussedWithContact
	...
	public boolean checkSummaryInformationDiscussedWithContact() {
		utils.clickAnelemnt(this.lblSummaryTab, "ConsumerDrivenHealthPlan", "Summary Tab");
		utils.waitforpageload();
		return utils.clickAnelemnt(this.summaryCheckBox, "ConsumerDrivenHealthPlan",
				"Summary information discussed with contact");
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:
	 * uncheckTransactionInformationDiscussedWithContact #Description:This method
	 * unchecks the checkbox displayed under Transaction tab. Type:TextBox
	 */
	public boolean uncheckTransactionInformationDiscussedWithContact() {
		return utils.clickAnelemnt(this.txCheckBox, "ConsumerDrivenHealthPlan",
				"Transaction information discussed with contact");
	}

	/*
	 * @SCU #CommonMethodwithoutArgument:checkCustodianBankInfoDiscussedWithContact
	 * #Description:This method checks the checkbox displayed under Custodian Bank
	 * tab. Type:TextBox
	 */
	public boolean checkCustodianBankInfoDiscussedWithContact() {
		if(utils.clickAnelemnt(this.lblCustodianBankTab, "ConsumerDrivenHealthPlan", "Custodian Bank"))
			return utils.clickAnelemnt(this.custBankCheckBox, "ConsumerDrivenHealthPlan",
					"Custodian Bank information discussed with contact");
		return false;
	}
	 ...
	 * #Description:This method unchecks the checkbox displayed under Custodian Bank
	 * tab. Type:TextBox
	 */
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:checkCardInformationDiscussedWithContact
	 * #Description:This method checks the checkbox displayed under Card Information
	 * tab. Type:TextBox
	 */
	public boolean checkCardInformationDiscussedWithContact() {
		if(utils.clickAnelemnt(this.lblCardInfoTab, "ConsumerDrivenHealthPlan", "Card Information"))
			return utils.clickAnelemnt(this.cardInfoCheckBox, "ConsumerDrivenHealthPlan",
					"Cards information discussed with contact");
		return false;

	}

	/*
	 * @SCU #CommonMethodwithoutArgument:uncheckCardInformationDiscussedWithContact
	 * #Description:This method unchecks the checkbox displayed under Card
	 * Information tab. Type:TextBox
	 */
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:clickOnSubmitInCDHP #Description:This
	 * method clicks on submit button in 'CDHP' page. Type:TextBox
	 */
	public boolean clickOnSubmit() {
		utils.scrolltomiddle();
		if(utils.clickAnelemnt(btnSubmit, "View Consumer Driven Health Plan","Click Submit"))
	...
	public void setCardTransactionDetails(HashMap<String, String> cardTransactionDetails) {
	...
	/*
	 * @SCU #CommonMethodwithArgument:selectGAInCDHPAccount #Description:This method
	 * selects Grievance and Appeals indicator for a 'Consumer Driven Health Plan
	 * Account', depending on the parameters passed for selection.
	 * #Arguments:CDHP-GAAccount-Key Type:Table Keys:#Account
	 * Description#Status#Effective Date#Plan Start Date#Plan End Date
	 */
	 ...
	 * #Description:This method validates the hover text displayed on hovering
	 * Grievance and Appeals Icon in CDHP Page Type:Textbox
	 */
	public boolean verifyHoverTextInCDHPGrievanceAndAppealsIcon() {
		if(utils.clickAnelemnt(hoverImgGAIcon, "CDHP", "hoverImgGAIcon")){
	...
	/*
	 * @SCU #CommonMethodwithoutArgument:validateGAMsgInCDHP #Description:This
	 * method validates message being displayed if 'Grievance and Appeals Indicator'
	 * is checked, but no CDHP account is not selected - inhibits user from
	 * proceeding to CDHP Review Page Type:Textbox
	 */
	public boolean validateGAMsgInCDHP() {
		return utils.validateLabel(errGAText, "You have either tagged Grievance and Appeal, Account Summary, Transactions, Transaction Details, Bank Information, or Card Information under an Account as being discussed. Tag the associated Account and Transaction if applicable and click Submit.");
	}

	/*
	 * @SCU #CommonMethodwithArgument:nextPageOfTransactionTable #Description:This
	 * method selects a navigates the user to a particular page in transaction
	 * records for a 'CDHP Account'. #Arguments:pageNo Type:Textbox
	 * Keys:#Transaction Date#Service Start
	 * Date#Account#Claimant#Type#Description#Status#Transaction Amount
	 */
	public boolean nextPageOfTransactionTable(String[] args) {
		if(utils.clickAnelemnt(this.lblTransactionTab, "ConsumerDrivenHealthPlan", "Transactions tab")) {
			utils.waitforpageload();
	...
	@FindBy(xpath = "//table[@pl_prop='D_CDHPTXInfo.pxResults']//div[@node_name='pyGridNoResultsMessage']//div[contains(text(),'No items')]")
	WebElement txtInTransactionsTab;

	@FindBy(xpath="//table[@pl_prop='D_CDHPCardInfo.pxResults']//div[@node_name='pyGridNoResultsMessage']//div[contains(text(),'No items')]")
	WebElement txtInCardInfoTab;

	public boolean selectTransactionTabInCDHPAccount() {
		return utils.clickAnelemnt(this.lblTransactionTab, "ConsumerDrivenHealthPlan", "Transactions tab");
	}

	public boolean selectCardDtailTabInCDHPAccount() {
		return utils.clickAnelemnt(this.lblCardInfoTab, "ConsumerDrivenHealthPlan", "Card Info tab");
	}

	public boolean verifyNoItemsMsgIsDisplayedInTransactionSection(){
		return utils.validateLabel(txtInTransactionsTab, "No items");
	}

	public boolean verifyNoItemsMsgIsDisplayedInCardDEtailsSection(){
		return utils.validateLabel(txtInCardInfoTab, "No items");
	}

	@FindBy(xpath="//em[starts-with(text(),'This member does not have an integrated')]")
	WebElement labelInstructionalTextForCDHPOptOut;

	@FindBy(xpath="//div[@title='Disclose Historical Consumer Driven Health Plan Accounts']")
	WebElement lnkHistoricalCommunicationCollapseIcon;


	public boolean verifyTheInstTextForCDHPOptOutMembers()
	{
		...
		String actualText = "This member does not have an integrated HSA with Anthem as of  "+date1+" they are using an outside HSA Custodian. Go to the vendor information section on the contract tab under the member composite to find to see if the member has Opted out Anthem CDH custodian to refer the member to.";
		System.out.println("Actual Text: "+actualText);
		String expectedText = labelInstructionalTextForCDHPOptOut.getText().replaceAll(",", "").replaceAll("/n", "");
		System.out.println("Expected Text: "+expectedText);
		return utils.isvalueMatch_contain(actualText, expectedText);

	}

	public boolean verifyAndExpandHistoricalCDHPAccountsSection()
	{
		if(!utils.isProxyWebelement(lnkHistoricalCommunicationCollapseIcon))
			return utils.clickAnelemnt(lnkHistoricalCommunicationCollapseIcon, "ConsumerDrivenHealthPlan", "Expand Historical Communication");
		return false;
	}

	@FindBy(xpath="//table[@pl_prop='D_CDHPAccountSummary.pxResults']")
	WebElement tblCDHPAccountSummary;


	@FindBy(xpath="//div[@node_name='NoCDHPInfo']//p")
	WebElement labelInstructionalTextForMemberDoesNotHaveAlegeusProduct;

	@FindBy(xpath="//li[@title='Transactions']")
	WebElement tabTransactions;

	@FindBy(xpath="//li[@title='Card Information']")
	WebElement tabCardInformation;

	public boolean verifyTheInstTextForMemberDoesNotHaveAlegeusProduct()
	{
		DateFormat dateFormat = new SimpleDateFormat("MMM dd yyyy");
		Date date = new Date();
		System.out.println(dateFormat.format(date));
		String date1 = dateFormat.format(date);
		System.out.println("Date1: "+date1);
		String actualText = "This member doesn’t have Anthem CDH or Lumenous as of "+date1+" go to the vendor information section on the contract tab under the member composite to find to see if the member has another CDHP vendor to refer the member to.";
		System.out.println("Actual Text: "+actualText);
		String expectedText = labelInstructionalTextForMemberDoesNotHaveAlegeusProduct.getText().replaceAll(",", "").replaceAll("\n", "");
		System.out.println("Expected Text: "+expectedText);
		return utils.isvalueMatch_contain(actualText, expectedText);
	...
	/**
	 * Verifies the row values given by the user and expands the matched row in the CDHP Account Summary Table
	 * @param args
	 * @return
	 * @throws InterruptedException
	 */
	public boolean expandTheCDHPAccount(String[] args) throws InterruptedException
	{
		utils.waitforpageload();
		WebElement row = utils.getTablerowbasedonvalues(tblCDHPAccountSummary, args);
	...
	/**
	 * validates the CDHP Account and tag the CDHP in the Account Summary table
	 * @param args
	 * @return
	 * @throws InterruptedException
	 */
	public boolean validateTheCDHPAccountAndTagTheCDHPAccount(String[] args) throws InterruptedException
	{
		WebElement row = utils.getTablerowbasedonvalues(tblCDHPAccountSummary, args);
	...
	/**
	 * Clicks the Transaction tab under the Expanded CDHP
	 * @return
	 */
	public boolean clickTransactionTab()
	{
		return utils.clickAnelemnt(tabTransactions, "ConsumerDrivenHealthPlan", "Transactions Tab");
	}

	/**
	 * Clicks the Card Information tab under the Expanded CDHP
	 * @return
	 */
	public boolean clickCardInformationTab()
	{
		return utils.clickAnelemnt(tabCardInformation, "ConsumerDrivenHealthPlan", "Card Information Tab");
	}

	@FindBy(id="CDHPSummaryReason")
	private WebElement CDHPSummaryReason;

	@FindBy(id="TransactionReason1")
	private WebElement CDHPTransactionReason;

	@FindBy(id="CDHPCardInfoReason")
	private WebElement CDHPCardReason;


	/**This method selects the summary details discussed dropdown
	 * 
	 * @param args
	 * @return
	 */

	public boolean selectSummaryDetailsDiscussedDropdownForCapitalMembers(String[] args){
		utils.waitforpageload();
		return utils.selectDropDownbyVisibleString(CDHPSummaryReason, args[0], "ViewCDHPAccounts", "Summary Details Discussed");   
	}

	/**This method selects the transaction details discussed dropdown
	 * 
	 * @param args
	 * @return
	 */
	public boolean selectTransactionDetailsDiscussedDropdownForCapitalMembers(String[] args){
		utils.waitforpageload();
		return utils.selectDropDownbyVisibleString(CDHPTransactionReason, args[0], "ViewCDHPAccounts", "Transaction Details Discussed");   
	}

	/**This method selects the card details discussed dropdown
	 * 
	 * @param args
	 * @return
	 */
	public boolean selectCardDetailsDiscussedDropdownForCapitalMembers(String[] args){
		utils.waitforpageload();
		return utils.selectDropDownbyVisibleString(CDHPCardReason, args[0], "ViewCDHPAccounts", "Card Details Discussed");   
	}

	@FindBy(xpath="//div[@id='DialogContent']")
	WebElement GuidedDialogueForFSA;
	/**
	 * This method validates the guided dialog displayed in Consumer driven page for FSA members
	 * @param args
	 * @return
	 */
	public boolean validateGuidedDialogueForFSA(String[] args)
	{
		return utils.isvalueMatch_contain(args[0], GuidedDialogueForFSA.getText().replaceAll("\n", ""));
	}


	@FindBy(xpath="//div[@id='DialogContent']")
	WebElement GuidedDialogueForLites;

	/**
	 * This method validates the guided dialog displayed in Consumer driven page for Lites members
	 * @param args
	 * @return
	 */
	public boolean validateGuidedDialogueForLites(String[] args)
	{
		return utils.isvalueMatch_contain(args[0], GuidedDialogueForLites.getText().replaceAll("\n", ""));
	}


	@FindBy(xpath="//div[@id='DialogContent']")
	WebElement GuidedDialogueForVendorAlegeus;

	/**
	 * This method validates the guided dialog displayed in Consumer driven page for VendorAlegeus members
	 * @param args
	 * @return
	 */
	public boolean validateGuidedDialogueForVendorAlegeus(String[] args)
	{
		return utils.isvalueMatch_contain(args[0], GuidedDialogueForVendorAlegeus.getText().replaceAll("\n", ""));
	}


	@FindBy(xpath="//div[@id='DialogContent']")
	WebElement GuidedDialogueForAlegeusOptout;
	/**
	 * This method validates the guided dialog displayed in Consumer driven page for AlegeusOptout members
	 * @param args
	 * @return
	 */
	public boolean validateGuidedDialogueForAlegeusOptout(String[] args)
	{
		return utils.isvalueMatch_contain(args[0], GuidedDialogueForAlegeusOptout.getText().replaceAll("\n", ""));
	}
