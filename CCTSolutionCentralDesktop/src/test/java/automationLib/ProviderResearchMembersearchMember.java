...
public class ProviderResearchMembersearchMember extends Driver {

	SeleniumUtilities utils = new SeleniumUtilities();
	ErrorLogger err = new ErrorLogger();
	BaseLogger blogger = new BaseLogger();
	Actions action=new Actions(Driver.pgDriver);
	public ProviderResearchMembersearchMember()
	...
	public boolean clickMSubmit()
	{
		...
		{
			try{
				//Thread.sleep(4000);)seO
				//Checking for the Next Page, if Present or not else an exception will be thrown 
				//getHeaderforTabsVerification("Member Verification");
			}
			catch(Exception e)
			{
				try
	...
	{
		return sHeader.contains(sHeaderSearchForMember.getText());
	}
	...
	{
		try {
			boolean flag =rBtnMID.isDisplayed();
	...
	public boolean selectfirstMember(){
		try {
			boolean flag =rdoFirstMemberRow.isDisplayed();
	...
	public boolean searchAndSubmitMember(String[] args) throws InterruptedException {
		...
		if(flag) {
			blogger.loginfo("PASS: Search and Submit provider Member successful");
			return true;
		}
		else {
			blogger.loginfo("FAIL: Search and Submit provider Member not successful");
			return false;
		}

	}

	public boolean MemberSearchPageNotDisplayed() {
		return !utils.isProxyWebelement(rBtnMID);
	}

	public boolean DisplayMemberSearchPage() {
		...
		try {
			boolean flag = rBtnMID.isDisplayed();
			if(flag) {
				blogger.loginfo("PASS: Member search page is displayed");
				return true;
			}else {
				blogger.loginfo("FAIL: Member search page should not be displayed");
				return false;	
			}
	...
	@FindBy(xpath="//label[@for='ContinueWithoutMember']/preceding-sibling::input[@type='checkbox']")
	WebElement contWithoutMember;

	@FindBy(xpath="//*[@class='pzbtn-mid'][text()='Submit']")
	WebElement submit;

	public boolean clickContinueWithoutMemberCheckbox() throws InterruptedException
	{
		utils.waitforpageload();
		if(utils.clickAnelemnt(contWithoutMember, "Search Member", "Continue withour Mmeber"))
			return utils.clickAnelemnt(submit, "search member", "Continue without member");
		return false;
	}
	...
	/**Verify HIPAA Verification And DIsclosure Link Is Not Available Under Search By at Search for Member page in Research Provider flow
	 * 
	 * @return
	 */
	public boolean verifyHIPAAVerificationAndDIsclosureNotAvailableMemberSearch(){
	...
	public boolean clickrbtnIsthisanITSHOSTAccountmemberYes()
	{
		if(ITSHostAccountYes.isDisplayed())
			if(utils.clickAnelemnt(ITSHostAccountYes, "ProviderResearchMembersearchMember", "Yes Radio Button"))
		return true;
		return false;
	}
	...
	public boolean clickrbtnIsthisanITSHOSTAccountmemberNo()
	{
		JavascriptExecutor jse = (JavascriptExecutor)Driver.pgDriver;
		jse.executeScript("window.scrollBy(0,450)","");
		if(utils.clickAnelemnt(ITSHostAccountNo, "ProviderResearchMembersearchMember", "No Radio Button"))
		return true;
		return false;
	}
	public boolean clickContinueWithoutMember() throws InterruptedException
	{
		utils.waitforpageload();
		if (utils.clickAnelemnt(contWithoutMember, "ProviderResearchMembersearchMember", "Continue withour Mmeber"))
	...
	{
		utils.waitforpageload();
		if (utils.clickAnelemnt(Submit, "ProviderResearchMembersearchMember", "Continue withour Mmeber")){
			utils.waitforpageload();
			return true;
	}
		return false;
	...
	/**This functionality selects the value given by the user in the ITS Host DropDown at the Member search screen in Research provider flow
	 * 
	 * @param args
	 * @return
	 */
	public boolean selectITSHostRegion(String args[]){
		if(utils.clickAnelemnt(this.ITSHOSTDropDOwn, "ProviderResearchMembersearchMember", "ITS Host Dropdown")) {
			...
			if(utils.checkvaluesinDropDown(this.ITSHOSTDropDOwn,dropdownValues))
				return utils.selectDropDownbyVisibleString(this.ITSHOSTDropDOwn, args[0], "ProviderResearchMembersearchMember", "ITS Host Dropdown");
		}
		return false;
	}
	...
	//Validates that the Transfer Interaction BTN is not Available at the Member search screen for Research provider flow
