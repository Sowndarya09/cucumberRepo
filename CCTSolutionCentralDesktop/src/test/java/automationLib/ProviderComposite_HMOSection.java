...
public class ProviderComposite_HMOSection {
	SeleniumUtilities utils = new SeleniumUtilities();
	ErrorLogger err = new ErrorLogger();
	BaseLogger blogger = new BaseLogger();
	Actions action = new Actions(Driver.pgDriver);

	public ProviderComposite_HMOSection() {
...
return false;
	...
	@FindBy(xpath="//span[text()='Office Co-pay']")
	WebElement OfficeCopay;
	...
	public boolean validateOfficeCopay(String[] args) {
		if(utils.clickAnelemnt(OfficeCopay,"ProviderComposite_HMOSection", "OfficeCopay"))
	...
	public boolean validateSiteDetails(String[] args) {
		...
		if(utils.clickAnelemnt(SiteDetails ,"ProviderComposite_HMOSection", "SiteDetails"))
			if(utils.validatetablerowbasedonvalues(OfficeDaysandHours, args[0]))
				if(utils.validatetablerowbasedonvalues(ListOfLanguages, args[1]))
					if(utils.clickAnelemnt(ClickAdditionalInfo, "ProviderComposite_HMOSection", "ClickAdditionalInfo" ))
						if(utils.isvalueMatch_compareto(s, args[2]))
					if(utils.validateLabel(AdditionalInfo, args[2]))
						if(utils.clickAnelemnt(ClickUrgentInfo, "ProviderComposite_HMOSection", "ClickUrgentInfo" ))
	...
	public boolean validateEnrollmentProtection(String[] args) {
		if(utils.clickAnelemnt(EnrollmentProtection ,"ProviderComposite_HMOSection", "EnrollmentProtection"))
	...
	{
		if(utils.clickAnelemnt(NetworkIDInformation ,"ProviderComposite_HMOSection", "NetworkIDInformation"))
...
return false;
	...
	{
		if(utils.clickAnelemnt(Membership ,"ProviderComposite_HMOSection", "NetworkIDInformation"))
