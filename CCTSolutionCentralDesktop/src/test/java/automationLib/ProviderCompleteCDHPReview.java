...
public class ProviderCompleteCDHPReview {
	 ...
	 */
	public ProviderCompleteCDHPReview() throws IOException
	{
	...
	public boolean completeCDHPReviewPage(String args[]){
			if(utils.selectDropDownbyVisibleString(this.reasonForContactCDHP, args[0], "CompleteCDHPReview", "Reason for Contact"))
				if(utils.clickAnelemnt(btnSubmit, "ProviderCompleteCDHPReview", "Submit"))
					return true;
			return false;
	}
	 ...
	 */
public boolean validateItemsDiscussedDuringCDHPReview(String args[]){
	if(this.itemsReviewedDuringCDHPReviewLbl.getText().equalsIgnoreCase("Items Reviewed During CDHP Review")){
	System.out.println("Items Reviewed During CDHP Review, is displayed on Complete CDHP Review page");
		boolean returnvar;
		returnvar = true;
			String keyvaluepair="";
			for(String iterator : args)
			{
				if(!returnvar)
				{
					err.logcommonMethodError("validateItemsDiscussedDuringCDHPReview", "Check your "+keyvaluepair+" Either Your input is wrong or the value found on application is incorrect");
					return false;
				}
				...
				if(key.toLowerCase().contains("anthem cdh link")){
					if(value.equalsIgnoreCase("yes")){
						returnvar = this.anthemcdhlinkAccessed.isDisplayed();
					}else if(value.equalsIgnoreCase("no")){
						try{
							returnvar = this.anthemcdhlinkAccessed.isDisplayed();
							return false;
						}catch(Exception e){
							err.logError("CompleteCDHPReview", "Exception occured while retrieving 'Anthem CDH Link Accessed'");
							returnvar = true;
						}
					}else{
						return false;
					...
					continue;}
				else if(key.toLowerCase().contains("tier i / tier ii document link")){
					if(value.equalsIgnoreCase("yes")){
						returnvar = this.tier1or2linkAccessed.isDisplayed();
					}else if(value.equalsIgnoreCase("no")){
						try{
							returnvar = this.tier1or2linkAccessed.isDisplayed();
							return false;
						}catch(Exception e){
							err.logError("CompleteCDHPReview", "Exception occured while retrieving 'Tier I / Tier II Document link was accessed'");
							returnvar = true;
						}
					}else{
						return false;
					...
					continue;}
				else if(key.toLowerCase().contains("consumer account database")){
					if(value.equalsIgnoreCase("yes")){
						returnvar = this.cadlinkAccessed.isDisplayed();
					}else if(value.equalsIgnoreCase("no")){
						try{
							returnvar = this.cadlinkAccessed.isDisplayed();
							return false;
						}catch(Exception e){
							err.logError("CompleteCDHPReview", "Exception occured while retrieving 'Consumer Account Database (CAD) was accessed'");
							returnvar = true;
						}
					}else{
						return false;
		...
		err.logcommonMethodError("CompleteCDHPReview", "validateItemsDiscussedDuringCDHPReview - Items Reviewed During CDHP Review isnt displayed");
		return false;
		}
	...
	public boolean verifyCDHPTransactionLevelInfoIsTagged(String args[]){
		try{
			...
			compareAdjudicationDetails();
			compareClaimComments();
			compareReimbursementDetails();
			compareClaimLineDetails();	
		}
		catch (Exception e){
	...
	//Fields for Custodian Bank Information
	...
	public HashMap<String, String> retrieveValuesFromAdjudicationDetails(){
		adjudicationDetailsReviewed = new HashMap<String,String>();
		hdrAdjudicationDetails.click();
		adjudicationDetailsReviewed.put("Service Dates", this.txt_service_dates.getText());
		adjudicationDetailsReviewed.put("Service Type", this.txt_service_type.getText());
		adjudicationDetailsReviewed.put("Service Detail Description", this.txt_service_details_desc.getText());
		adjudicationDetailsReviewed.put("Total Amount", this.txt_total_amount.getText());
		adjudicationDetailsReviewed.put("Pended", this.txt_pended.getText());
		adjudicationDetailsReviewed.put("Denied", this.txt_denied.getText());
		adjudicationDetailsReviewed.put("Excluded", this.txt_excluded.getText());
		adjudicationDetailsReviewed.put("Eligible", this.txt_eligible.getText());
		adjudicationDetailsReviewed.put("Applied to Account Deductible", this.txt_applied_to_account_Deductible.getText());
		adjudicationDetailsReviewed.put("Offset", this.txt_offset.getText());
		adjudicationDetailsReviewed.put("Low Funds", this.txt_low_funds.getText());
		adjudicationDetailsReviewed.put("Approved", this.txt_Approved.getText());
		return adjudicationDetailsReviewed;
	}
	//Claim Comments
	...
	public HashMap<String, String> retrieveValuesFromClaimComments(){
		claimCommentsReviewed = new HashMap<String,String>();
		hdrClaimComments.click();
		claimCommentsReviewed.put("Denial Reason", this.txt_denial_reason.getText());
		claimCommentsReviewed.put("Denied Comment", this.txt_denial_comment.getText());
		claimCommentsReviewed.put("Pended Reason", this.txt_pended_reason.getText());
		claimCommentsReviewed.put("Pended Comment", this.txt_pended_comment.getText());
		return claimCommentsReviewed;
	}
	...
	public HashMap<String, String> retrieveValuesFromReimbursementDetails(){
		reimbursementDetailsReviewed = new HashMap<String,String>();
		hdrReimbursementDetails.click();
		reimbursementDetailsReviewed.put("Reimbursement Date", this.txt_reimb_date.getText());
		reimbursementDetailsReviewed.put("Reimbursement Method", this.txt_reimb_method.getText());
		reimbursementDetailsReviewed.put("Amount Reimbursed", this.txt_amount_reimb.getText());
		reimbursementDetailsReviewed.put("Reissued Check", this.txt_reissued_check.getText());
		reimbursementDetailsReviewed.put("Check / Trace Number", this.txt_check_trace_no.getText());
		return reimbursementDetailsReviewed;
	}
...
@FindBy(className="actionTitleBarLabelStyle")
	...
	public boolean clickOtherActionRequestManagerHelp(){
		if(utils.clickAnelemnt(this.btnOtherActions, "CompleteCDHPReview", "Other Actions button"))
			if(utils.clickAnelemnt(this.lnkOtherReqMgrHelp, "CompleteCDHPReview", "Request Manager Help"))
				utils.waitforpageload();
				if(utils.validateHeader(this.sHeaderCompleteCDHP,"Request Manager Help"))
					return true;
			return false;
	}
	...
	public boolean verifyOtherActionsDoesntDisplayRequestGA(){
			if(utils.clickAnelemnt(this.btnOtherActions, "CompleteProviderReview", "Other Actions button"))
				if(utils.isProxyWebelement(lnkOtherRequestGandA))
					return true;
				return false;
		}
	...
	public boolean validateGAMsgInCDHP(){
			...
			return utils.isvalueMatch_contain(errGAText, expectedGAText);
	}
