...
public class API_Contacts {
	...
	public boolean getContacts(String[] args) throws IOException, InterruptedException{
		String muid = getMemberUID(args);

		String siturl = "https://uat.api.anthem.com/v2/pegadesktop/members/contacts?mbruid="+muid;
	...
	public boolean validatePhoneType(String[] args){
		Boolean flag = true;
		while(parser.hasNext()){
			Event e=parser.next();
			if(e==Event.KEY_NAME && parser.getString().equals("phoneTypeCd")){
				parser.next();
				for(String arg:args) {
					e=parser.next();
