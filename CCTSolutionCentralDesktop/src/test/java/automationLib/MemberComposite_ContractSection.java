...
public class MemberComposite_ContractSection {
	WebDriverWait wait;
	...
	public MemberComposite_ContractSection() {
	...
	@FindBy(xpath = "//*[@data-test-id='201509101229430562277900']")
	...
	@FindBy(xpath = "//div[@node_name='CPMCompositesContainer']//h3[text()='Policy']")
	...
	public boolean switchContractAndVerifyTheSwitchContract() throws InterruptedException {
		if(clickContractInContractSection()) {
			...
			if (!utils.isProxyWebelement(contractCodeInPopup)) {
				...
				if(utils.clickAnelemnt(contractCodeInPopup, "Contract Section", "contractCodeInPopup")) {
					utils.waitforpageload();
					if(utils.clickAnelemnt(tabContract, "Member Composite ", "Contract tab "))
			...
			if (!utils.isProxyWebelement(planAdminExpandIcon)) {
				...
				for (String iterator : mgeninfoDetails) {
					...
					if (key.toLowerCase().contains("contractcode")) {
						returnvar = contractCodeContractInfo.getText().toLowerCase().contains(value);
						continue;
					} else if (key.toLowerCase().contains("coverage")) {
						returnvar = coverageTypeContractInfo.getText().toLowerCase().contains(value);
						continue;
					} else if (key.toLowerCase().contains("benefitperiod")) {
						returnvar = benifitPeriodContractInfo.getText().toLowerCase().contains(value);
						continue;
					} else if (key.toLowerCase().contains("asofdate")) {
						returnvar = asOfDtContractInfo.getText().toLowerCase().contains(value);
						continue;
					} else if (key.toLowerCase().contains("anthemcode")) {
						returnvar = anthemCodeContractInfo.getText().toLowerCase().contains(value);
						continue;
						...
						returnvar = networkNameContractInfo.getText().toLowerCase().contains(value);

						continue;
					} else if (key.toLowerCase().contains("policystate")) {
						returnvar = policyStateContractInfo.getText().toLowerCase().contains(value);
						continue;
	...
	public boolean verifyPlanAdministration(String[] mgeninfoDetails) throws Exception {
		...
		for (String iterator : mgeninfoDetails) {
			...
			if (key.toLowerCase().contains("acagrandfathered")) {
				returnvar = this.ACAGrandfatheredInPolicy.getText().toLowerCase().contains(value);
				continue;
			} else if (key.toLowerCase().contains("submitclaimswithin")) {
				returnvar = this.submintClaimsWithInPolicy.getText().toLowerCase().contains(value);
				continue;

			} else if (key.toLowerCase().contains("dependentmaxagelimit")) {
				returnvar = this.DependentMaxAgeLimitInContract.getText().toLowerCase().contains(value);
				continue;
			} else if (key.toLowerCase().contains("domesticpartner")) {
				returnvar = this.DomesticPartnerInContract.getText().toLowerCase().contains(value);
				continue;
			} else
				err.logcommonMethodError("Member Composite",
						"Check your key in yourkeypair. Make sure you are following the same pattern for input");
	...
	public boolean verifySubmitClaimsWithinHoverText(String hoverMessage[]) {
		if (mouseHover(dependentMaxAgeLimithovertext, "Member Composite", "Member Tab")) {
			String hovertext = dependentMaxAgeLimithovertext.getAttribute("aria-label").toString().toLowerCase();
